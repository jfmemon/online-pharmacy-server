...
async function run() {
  try {
    ...
    const shopByConditionCollection = client
      .db("online-pharmacy-db")
      .collection("shopByCondition");
    const sexualWellnessCollection = client
      .db("online-pharmacy-db")
      .collection("sexualWellness");
    const birthControlCollection = client
      .db("online-pharmacy-db")
      .collection("birthControl");
    const vitaminsAndSupplementsCollection = client
      .db("online-pharmacy-db")
      .collection("vitaminsAndSupplements");
    const medicalDevicesCollection = client
      .db("online-pharmacy-db")
      .collection("medicalDevices");
    const personalCareCollection = client
      .db("online-pharmacy-db")
      .collection("personalCare");
    const healthAndWellnessCollection = client
      .db("online-pharmacy-db")
      .collection("healthAndWellness");
    const babyCareCollection = client
      .db("online-pharmacy-db")
      .collection("babyCare");
    ...
    app.delete("/sexualWellness/:id", async (req, res) => {
      const id = req.params.id;
      const query = { _id: new ObjectId(id) };
      const result = await sexualWellnessCollection.deleteOne(query);
      res.send(result);
    });
    ...
    app.post(
      ...
      async (req, res) => {
        const newItem = req.body;
        const result = await vitaminsAndSupplementsCollection.insertOne(
          newItem
        );
        res.send(result);
      }
    ...
    app.delete("/healthAndWellness/:id", async (req, res) => {
      const id = req.params.id;
      const query = { _id: new ObjectId(id) };
      const result = await healthAndWellnessCollection.deleteOne(query);
      res.send(result);
    });

    app.post("/healthAndWellness", verifyJWT, verifyAdmin, async (req, res) => {
      const newItem = req.body;
      const result = await healthAndWellnessCollection.insertOne(newItem);
      res.send(result);
    });
    ...
    app.get("/allContacts", verifyJWT, verifyAdmin, async (req, res) => {
    ...
    app.get("/contacts", async (req, res) => {
      ...
      const query = { "user.email": email }; // Assuming 'user' is the key for the user object
